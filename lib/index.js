...
(function() {
  'use strict';
  var BufferStreams, PLUGIN_NAME, URL_REGEX, cleanMatch, es, green, gutil, isRelativeToBase, isRelativeUrl, magenta, path, url;
  ...
  module.exports = function(opt) {
    var bufferReplace, ref, rewriteUrls, streamReplace;
    if (opt == null) {
      opt = {};
    }
    if (opt.debug == null) {
    ...
    if (opt.enhanced != null) {
      if (!(((ref = opt.enhanced) != null ? ref.re : void 0) && opt.enhanced.re instanceof RegExp)) {
        throw new gutil.PluginError(PLUGIN_NAME, 'enhanced.re must be an instance of RegExp');
      }
      if (opt.enhanced.subst == null) {
    ...
    rewriteUrls = function(sourceFilePath, data) {
      ...
      return data.replace(URL_REGEX, function(match, file) {
        ...
        if (opt.enhanced != null) {
          ...
          if (opt.debug) {
            gutil.log(magenta(PLUGIN_NAME), 'rewriting path for', magenta(file), 'to', magenta(ret), 'in', green(sourceFilePath));
          }
        } else {
